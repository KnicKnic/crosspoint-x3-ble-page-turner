...
bool equalsCaseInsensitive(const std::string& a, const char* b) {
  ...
  return containsCaseInsensitive(a, b);
}
}
...
void BluetoothSettingsActivity::loop() {
  ...
  if (btMgr && reconnectScanPending && !btMgr->isScanning() && lastScanTime > 0) {
    if (millis() - lastScanTime > 500) {
      lastScanTime = 0;
      reconnectScanPending = false;
      if (finishBondedReconnectScan()) {
        return;
      }
      requestUpdate();
      return;
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
    ...
    if (selectedIndex == kToggleBluetoothIndex) {
      ...
      if (btMgr->isBondedReconnectInProgress()) {
        ...
        lastError = "No bonded remote";
      } else if (btMgr->startBondedReconnect(12000)) {
        reconnectJobPending = true;
...
void BluetoothSettingsActivity::startBondedReconnectScan() {
  if (!btMgr) {
    lastError = "BLE not available";
    return;
  }
  ...
  LOG_INF("BT", "Scanning for bonded remote %s (%s)", SETTINGS.bleBondedDeviceName, SETTINGS.bleBondedDeviceAddr);
  btMgr->setBondedDevice(SETTINGS.bleBondedDeviceAddr, SETTINGS.bleBondedDeviceName,
                          SETTINGS.bleBondedDeviceAddrType);
  reconnectScanPending = true;
  lastError = "Looking for remote...";
  btMgr->startScan(5000);
  lastScanTime = millis();
}

bool BluetoothSettingsActivity::finishBondedReconnectScan() {
  if (!btMgr) {
    lastError = "BLE not available";
    return false;
  }
  ...
  const BluetoothDevice* candidate = nullptr;
  for (const auto& device : devices) {
    if (matchesBondedDevice(device)) {
      candidate = &device;
      break;
    }
  }
  ...
  if (!candidate) {
    lastError = "Remote not found; wake it and retry";
    return false;
  }

  LOG_INF("BT", "Connecting to bonded scan match %s (%s type=%u)", candidate->name.c_str(),
          candidate->address.c_str(), static_cast<unsigned>(candidate->addressType));
  lastError = "Connecting...";
  requestUpdate();

  if (!btMgr->connectToDevice(candidate->address, 7000, candidate->addressType)) {
    lastError = btMgr->lastError.empty() ? "Reconnect failed" : btMgr->lastError;
    return false;
  }

  rememberBondedDevice(*candidate);
...
bool BluetoothSettingsActivity::matchesBondedDevice(const BluetoothDevice& device) const {
  if (SETTINGS.bleBondedDeviceAddr[0] != '\0' && device.address == SETTINGS.bleBondedDeviceAddr) {
    return true;
  }

  if (SETTINGS.bleBondedDeviceName[0] != '\0') {
    if (equalsCaseInsensitive(device.name, SETTINGS.bleBondedDeviceName) ||
        containsCaseInsensitive(device.name, SETTINGS.bleBondedDeviceName) ||
        containsCaseInsensitive(SETTINGS.bleBondedDeviceName, device.name.c_str())) {
      return true;
    }
...
void BluetoothSettingsActivity::rememberBondedDevice(const BluetoothDevice& device) {
  ...
  SETTINGS.bluetoothEnabled = 1;
  SETTINGS.saveToFile();
  btMgr->setBondedDevice(device.address, device.name, device.addressType);
}
...
void BluetoothSettingsActivity::handleDeviceListInput() {
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
    ...
    if (selectedIndex >= 0 && selectedIndex < static_cast<int>(devices.size())) {
      ...
      if (btMgr->connectToDevice(device.address, 10000, device.addressType)) {
        rememberBondedDevice(device);
...
void BluetoothSettingsActivity::renderDeviceList() {
  ...
  const bool scanning = btMgr->isScanning();

  // Header with device count
  char countStr[32];
  if (scanning) {
    snprintf(countStr, sizeof(countStr), "%s", tr(STR_SCANNING));
  } else {
    snprintf(countStr, sizeof(countStr), "Found %zu", btMgr->getDiscoveredDevices().size());
  }
  ...
  if (scanning) {
    deviceLabels.push_back("Scanning...");
...
void BluetoothSettingsActivity::renderDebugMonitor() {
  ...
  if (debugUniqueCount == 0) {
    ...
    const int uniqueCount = static_cast<int>(debugUniqueCount);
    int sortedIndices[kDebugUniqueKeyMax] = {0};
    ...
    const int renderCount = (uniqueCount < 4) ? uniqueCount : 4;
    ...
    if (uniqueCount > renderCount) {
      char moreLine[48];
      snprintf(moreLine, sizeof(moreLine), "+%u more keys", static_cast<unsigned>(uniqueCount - renderCount));
